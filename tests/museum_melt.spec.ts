...
test('Museum Melt Effect and Layout', async ({ page }) => {
  ...
  const display = await firstItem.evaluate((el) => window.getComputedStyle(el).display);
  ...
  // Get initial values
  const initialBlur = await firstItem.evaluate((el) => el.style.getPropertyValue('--blur'));
  ...
  // Get new values
  const newBlur = await firstItem.evaluate((el) => el.style.getPropertyValue('--blur'));
